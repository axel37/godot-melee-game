...
signal blocked_damage(time_since_block: float, damage_dealer: DamageDealer, dealer_shape_index: int, blocker_position: Vector3)
...
func _on_area_shape_entered(area_rid: RID, area: Area3D, area_shape_index: int, local_shape_index: int) -> void:
	...
	blocked_damage.emit(time_blocking, damage_dealer, area_shape_index, global_position)
